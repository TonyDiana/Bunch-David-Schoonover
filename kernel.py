...
# --- Constantes y declaraciones propias
__version__ = '1.0.2'
VERSION = tuple(map(int, __version__.split('.')))

__all__ = ('_Bunch', 'bunchify', 'unbunchify')
...
def bunchify(x):
    ...
    if isinstance(x, dict):
        return _Bunch((k, bunchify(v)) for k, v in iteritems(x))
...
try:
    ...
    def to_yaml(dumper, data):
        ...
        """
        return dumper.represent_mapping(unicod('!bunch.Bunch'), data)

    yaml.add_constructor(unicod('!bunch'), from_yaml)
    yaml.add_constructor(unicod('!bunch.Bunch'), from_yaml)

    SafeRepresenter.add_representer(_Bunch, to_yaml_safe)
    SafeRepresenter.add_multi_representer(_Bunch, to_yaml_safe)

    Representer.add_representer(_Bunch, to_yaml)
    Representer.add_multi_representer(_Bunch, to_yaml)
    ...
    _Bunch.toYAML = toYAML
    _Bunch.fromYAML = staticmethod(fromYAML)
